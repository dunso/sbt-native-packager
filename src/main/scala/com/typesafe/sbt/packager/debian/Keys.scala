...
trait DebianKeys {
  ...
  //- This is the changelog used by dpkg-genchanges to create the .changes file.
  //- This will allow you to upload the debian package to a mirror.
  ...
  //- DEPRECATED use maintainerScripts in Debian instead. These are the packaging scripts themselves used by dpkg-deb to build your debian.
  //- These scripts are used when installing/uninstalling a debian, like prerm, postinstall, etc. These scripts are placed in the DEBIAN file when building.
  //- Some of these files can be autogenerated, for example when using a package archetype, like server_application.
  //- However, any autogenerated file can be overridden by placing your own files in the src/debian/DEBIAN directory.
